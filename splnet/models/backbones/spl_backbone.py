...
class FrameStackBackbone(BaseModule):
    def __init__(self, 
        ...
        for i, m_out in enumerate(out_channels):
            # down stream
            self.sp_conv_list.append(self._make_sparse_layer(m_in, m_out, stride=strides[i], indice_key=f'stage_{i}'))
            self.down_trans_list.append(MultiHeadAttentionModule(m_out, m_out, mid_c=m_out, w_size=w_sizes[i], pos_embedding=pos_embedding, attn_dropout=attn_dropout))
            # fusion block used to fusing features from voxel-branch and point branch.
            # if i % 2 == 1:
            #     self.enc_point_list.append(self._make_point_layer(m_in + m_out, m_out))
            # else:
            #     self.enc_point_list.append(self._make_point_layer(m_in, m_out))

            # up stream
            self.sp_deconv_list.append(self._make_de_sparse_layer(m_out*2, m_in, stride=strides[i], indice_key=f'stage_{i}'))
            self.up_trans_list.append(MultiHeadAttentionModule(m_in, m_in, mid_c=m_in, w_size=w_sizes[i], pos_embedding=pos_embedding, attn_dropout=attn_dropout))

            m_in = m_out

        # m_in = out_channels[-1]
        # for i, m_out in enumerate(out_channels[:-1][::-1] + (self.in_channels,)):
        #     if i % 2 == 1:
        #         self.dec_point_list.append(self._make_point_layer(m_in + m_out, m_out))
        #     else:
        #         self.dec_point_list.append(self._make_point_layer(m_in, m_out))
    ...
    def forward(self, voxel_dict: dict) -> dict:
        ...
        for i, sp_conv in enumerate(self.sp_conv_list):
            # print(f'sp_conv: {sp_conv}; input: {x.spatial_shape}')
            x = sp_conv(x)
            ...
            # begin of fusion block.
            # transfer features from voxel to point within it.
            # if i % 2 == 1:
            #     pts_feats_from_voxel = self.voxel2point(x_tran, pts_coors, self.cum_strides[i])
            #     # (N, 2C) -> (N, C)
            #     point_feats = self.enc_point_list[i](torch.cat([point_feats, pts_feats_from_voxel], dim=-1))

            #     # transfer features from points to voxel by performing max pooling within each voxel.
            #     x_tran = self.point2voxel(point_feats, pts_coors, x_tran.shape, self.cum_strides[i])
            # else:
            #     point_feats = self.enc_point_list[i](point_feats)
            # end of fusion block.                
        ...
        for i, sp_deconv in enumerate(self.sp_deconv_list):
            x = sp_deconv(self.cat_sparse_tensor(x, out_spconv_list[-i-1]))
            ...
            # begin of fusion block in decode layer.
            # transfer features from voxel to point within it.
            # if i % 2 == 1:
            #     pts_feats_from_voxel = self.voxel2point(x_tran, pts_coors, self.inv_strides[i])
            #     # (N, 2C) -> (N, C)
            #     point_feats = self.dec_point_list[i](torch.cat([point_feats, pts_feats_from_voxel], dim=-1))
            #     # transfer features from points to voxel by performing max pooling within each voxel.
            #     x_tran = self.point2voxel(point_feats, pts_coors, x_tran.shape, self.inv_strides[i])
            # else:
            #     point_feats = self.dec_point_list[i](point_feats)    
    ...
    def cat_sparse_tensor(self, a, b):
        ...
        """
        # print(f'a.spatial_shape: {a.spatial_shape}; b.spatial_shape: {b.spatial_shape}')
        # print(f'a.indices: {a.indices.shape}; b.indices: {b.indices.shape}')
        ...
        if torch.equal(a.indices, b.indices):
            ...
            return b.replace_feature(feats)
        else:
            # (B, F, H, W, C) 
            ...
            indices = torch.nonzero(torch.any(fused_dense!=0, dim=-1))
            feats = fused_dense[indices[:, 0], indices[:, 1], indices[:, 2], indices[:, 3]]
            
            return SparseConvTensor(feats, indices, spatial_shape=a.spatial_shape, batch_size=a.batch_size)
